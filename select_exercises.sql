USE codeup_test_db;

SELECT 'Using codeup_test_db' AS "Database";


SELECT 'ALBUMS BY PINK FLOYD: ' AS INFO;
SELECT * FROM ALBUMS WHERE ARTIST = 'PINK FLOYD';


SELECT 'RELEASE YEAR OF SGT. PEPPER''S LONELY HEARTS CLUB BAND: ' AS INFO;
SELECT RELEASE_YEAR FROM ALBUMS WHERE NAME like '%pepper%';


SELECT 'GENRE OF NEVERMIND: ' AS INFO;
SELECT GENRE
FROM ALBUMS
WHERE NAME = 'NEVERMIND';


SELECT * 'ALBUMS RELEASED IN THE 1990''S' AS INFO;
SELECT NAME FROM ALBUMS
WHERE RELEASE_YEAR
BETWEEN 1990 AND 2000;


SELECT 'ALBUMS THAT HAD LESS THAN 20 MILLION CERTIFIED SALES: ' AS INFO;
SELECT NAME FROM ALBUMS
WHERE SALES < 20;


SELECT 'ALBUMS IN THE ROCK GENRE: ' AS INFO;
SELECT NAME FROM ALBUMS
WHERE GENRE = 'ROCK';


-- can organize data horizontally or vertically, as seen above.