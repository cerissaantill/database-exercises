...
-- 1. duplicate above query and update it to find all employees
--      whose last name starts AND ends with E
-- 2. use concat() to combine their first and last name together as a single
--      column in your result.

SELECT employee_no, birth_date, concat(first_name, ' ', last_name) AS 'full_name', gender, hire_date
FROM employees
WHERE last_name LIKE 'E%'
...
-- 3. use datediff() to find how many days they have been working at the company
--      [hint: you will use now() and curdate() ]
